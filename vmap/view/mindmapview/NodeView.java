 ...
 */
/*$Id: NodeView.java,v 1.8 2005/04/09 17:12:03 veghead Exp $ */
...
import vmap.modes.NodeAdapter;//This should not be done.
...
public abstract class NodeView extends JLabel {
    ...
    public boolean followLink(double xCoord) {
        return (( getModel().getLink() != null) ||
                (! getModel().getAttach().toString().equals(""))) &&
                    (getModel().isRoot() || 
                    !getModel().hasChildren() || 
                    xCoord < getSize().width/2); 
    }
...
//	    sibling = (NodeView)v.getLast(); // loop
	...
	for(ListIterator e = getChildrenViews().listIterator();e.hasNext();) {
     ...
     void update() {
        ...
        if (!(((NodeAdapter)getModel()).getAttach().toString().equals(""))) {
            MindIcon aticon = new MindIcon("fileicons/text");
            iconImages.addImage((ImageIcon)aticon.getIcon(map.getController().getFrame()));
        } 
                ...
                ImageIcon icon = new ImageIcon(((NodeAdapter)getModel()).getFrame().getResource
                                          (link.startsWith("mailto:") ? "images/Mail.png" :
                                           (Tools.executableByExtension(link) ? "images/Executable.png" :
                                            "images/icons/fileicons/html.png")));
...
//         }
        ...
        if (nodeText.startsWith("<html>")) {
            nodeText = nodeText.substring(6);
        } 

        if (nodeText.length() > 1) {
            nodeText = "<html>" + getModel().getTitle() + "<br>" + nodeText;
        } else {
            nodeText = "<html>" + getModel().getTitle();
        }
